...
def draw_stats(frame, tello, rc_commands):
    ...
    try:
        ...
        altitude_tof = state.get('tof', 'N/A')
        battery = state.get('bat', 'N/A')
        stats_text = f"Alt: {altitude_tof} cm | Bat: {battery}%"
...
def track_person(frame, results, frame_center_x, frame_center_y):
    ...
    if target_box:
        ...
        if len(area_history) > MAX_AREA_HISTORY:
            area_history.pop(0)
        ...
        if current_width > 0:
            ...
            cv2.putText(frame, f"Jarak: {distance:.2f} cm", (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        error_x = box_center_x - frame_center_x
        if abs(error_x) > TOLERANCE_X:
            yaw = (PID_P_YAW * error_x) + (PID_D_YAW * (error_x - previous_error_x))
...
log_file_name = f"tello_log_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.csv"
csv_file = open(log_file_name, 'w', newline='')
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['timestamp', 'detection_confidence', 'estimated_distance_cm', 'flight_time', 'battery', 'acc_x', 'acc_y', 'acc_z', 'pitch', 'roll', 'yaw_attitude', 'altitude_tof', 'vel_x', 'vel_y', 'vel_z', 'rc_lr', 'rc_fb', 'rc_ud', 'rc_yaw'])
...
try:
    ...
    if battery < MIN_BATTERY_TAKEOFF:
        raise Exception("Baterai di bawah ambang batas aman.")
    tello.streamon()
    ...
    while True:
        ...
        lr, fb, ud, yaw, confidence, est_distance = track_person(annotated_frame, results, frame_center_x, frame_center_y)

        if tello.get_height() > MAX_ALTITUDE_CM:
            ud = -20

        final_lr, final_fb, final_ud, final_yaw = send_rc_command(lr, fb, ud, yaw)

        current_timestamp = datetime.datetime.now().isoformat()
        drone_state = tello.get_current_state()
        data_row = [
            current_timestamp,
            confidence,
            est_distance,
            drone_state.get('time', ''),
            drone_state.get('bat', ''),
            drone_state.get('agx', ''),
            drone_state.get('agy', ''),
            drone_state.get('agz', ''),
            drone_state.get('pitch', ''),
            drone_state.get('roll', ''),
            drone_state.get('yaw', ''),
            drone_state.get('tof', ''),
            drone_state.get('vgx', ''),
            drone_state.get('vgy', ''),
    ...
    if 'tello' in locals() and tello.is_flying:
        try:
            ...
            print(f"Gagal mendarat normal: {e_land}.")
